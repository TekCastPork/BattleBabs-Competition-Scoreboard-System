...
namespace BattleBabs_Client
{
    ...
    class GameUtility
    {
        ...
        public static void setupObjects() // must be called before using any other functions!
        {
            ...
            RNGesusSeed = new Random(DateTime.Today.Millisecond); // use if you want to debug via seeding
            Net3.start();
        }
        ...
        public static void setRandomSong() // uses RNG to pick a new song to play, so long as it wasnt just played
        {
            ...
            {
                RNGResult = RNGesusSeed.Next(0, 7); // get the next value according to the RNG for use with picking the song
